(() => {
    ...
    function initialize(){
        ...
        if(isInit == 0){
            ...
            btn_left = new My_Button(ctx,"←",BTN_X,BTN_Y,BTN_WIDTH,BTN_HEIGHT);
            btn_right = new My_Button(ctx,"→",BTN_X+BTN_X_GAP,BTN_Y,BTN_WIDTH,BTN_HEIGHT);
            eventSetting();
        }
        isInit=1;
    ...
    function title(){
        ...
        ctx.font = '30px serif';
        ctx.fillText('厄を払え! 歳の神(仮)',CANVAS_WIDTH/5-25,CANVAS_HEIGHT/3);
    ...
    function eventSetting(){
        ...
        window.addEventListener('touchstart', (e)=>{
            ...
            if(scene == TITLE){
                ...
                mouse_X = touchObject.screenX;
                mouse_Y = Math.floor(touchObject.screenY);

                if(btn_left.isPushed(mouse_X,mouse_Y)){
                    window.isKeyDown.key_ArrowLeft = true;
                    btnStatus = 1;
                    console.log("left");
                }
                if(btn_right.isPushed(mouse_X,mouse_Y)){
        ...
        window.addEventListener('mousedown', (e)=>{
            if(scene == TITLE){
                ...
                if(btn_left.isPushed(offset_X,offset_Y)){
                    //isKeyDown[`key_ArrowLeft`] = true;
                    window.isKeyDown.key_ArrowLeft = true;
                    btnStatus = 1;
                    console.log("left");
                }
                if(btn_right.isPushed(offset_X,offset_Y)){
